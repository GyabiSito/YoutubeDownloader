...
RUN apt-get update \
    && apt-get install -y --no-install-recommends ffmpeg libicu-dev libonig-dev libzip-dev python3-pip \
    && docker-php-ext-install intl mbstring opcache zip \
    && pip3 install --break-system-packages --no-cache-dir --pre "yt-dlp[default]" "bgutil-ytdlp-pot-provider==1.3.1" \
    && a2enmod rewrite \
